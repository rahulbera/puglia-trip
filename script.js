...
function renderBeaches(beaches, filter = 'all') {
  ...
  $('#beach-grid').innerHTML = visible.map(beach => `
    <article class="beach-card">
      <div class="beach-image"><img src="${beach.image}" alt="${beach.imageAlt}" loading="lazy" decoding="async"><a class="card-cover-link" href="${beach.maps}" target="_blank" rel="noreferrer" aria-label="Open ${beach.name} in Google Maps"></a><span class="beach-badge">${beach.best}</span><span class="beach-rating">★ ${beach.rating.toFixed(1)}</span></div>
...
function renderPlaces(places) {
  $('#places-grid').innerHTML = places.map(place => `
    <article class="mini-card">
      <div class="mini-image"><img src="${place.image}" alt="${place.imageAlt}" loading="lazy" decoding="async"><a class="card-cover-link" href="${place.maps}" target="_blank" rel="noreferrer" aria-label="Open ${place.name} in Google Maps"></a><span>${place.best}</span></div>
...
function renderFood(foods) {
  $('#food-grid').innerHTML = foods.map(food => `
    <article class="mini-card">
      <div class="mini-image"><img src="${food.image}" alt="${food.imageAlt}" loading="lazy" decoding="async"><a class="card-cover-link" href="${food.maps}" target="_blank" rel="noreferrer" aria-label="Find ${food.name} near ${food.where} on Google Maps"></a><span>Try in ${food.where}</span></div>
