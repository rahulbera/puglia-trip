  ...
  $('#itinerary-list').innerHTML = days.map((day, index) => `
    <article class="day-card ${index === 0 ? 'open' : ''}">
      <button class="day-button" aria-expanded="${index === 0}" aria-controls="day-${day.day}">
        <span class="day-number"><small>Day</small>${String(day.day).padStart(2, '0')}</span>
        <span class="day-title"><b>${day.title}</b><span>${day.date} · ${day.subtitle}</span></span>
...
function renderBeaches(beaches, filter = 'all') {
  ...
  $('#beach-grid').innerHTML = visible.map(beach => `
    <article class="beach-card">
      <div class="beach-image ${beach.coast}"><span class="beach-badge">${beach.best}</span><span class="beach-rating">★ ${beach.rating.toFixed(1)}</span></div>
